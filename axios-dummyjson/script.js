...
function allproduk() {
    axios.get("https://dummyjson.com/products").then(function (response) {
        ...
        let out = '<table class="table table-secondary table-striped-columns"><thead><tr><th>No</th><th>Nama Barang</th><th>Deskripsi</th><th>Ubah</th><th>Hapus</th><th>Beli</th></tr></thead><tbody>';
        produk.forEach(el => {
            out += `<tr>
                    <td>${el.id}</td>
                    <td>${el.title}</td>
                    <td>${el.description}</td>
                    <td><button type="button" class="btn btn-outline-secondary fw-bolder" onclick="tampilproduk(${el.id})" data-bs-toggle="modal" data-bs-target="#exampleModal3">UBAH</button></td>
                    <td><button type="button" class="btn btn-outline-secondary fw-bolder" onclick="hapusproduk(${el.id})">HAPUS</button></td>
                    <td><button type="button" class="btn btn-outline-success fw-bolder" onclick="keranjang(${el.id})">Tambah Ke Keranjang</button></td>
            </tr>`;
        });
        out += '</tbody></table>';
...
function allprodukcate() {
    let kategori = document.getElementById("cate").value;
...
// Semua Pelanggan
function allplgn() {
    axios.get("http://localhost:8080/dummyjson-v2/php/select.php").then(function (response) {
        let pelanggan = response.data;
        let out = '<table class="table table-secondary table-striped-columns"><thead><tr><th>No</th><th>Nama Pelanggan</th><th>Alamat</th><th>Nomor Telepon</th><th>Ubah</th><th>Hapus</th><th>Beli</th></tr></thead><tbody>';
        pelanggan.forEach(el => {
            ...
            <td>${el.alamat}</td>
            <td>${el.telp}</td>
            <td><button type="button" class="btn btn-outline-secondary fw-bolder" onclick="tampilplgn(${el.idpelanggan})" data-bs-toggle="modal" data-bs-target="#exampleModal6">UBAH</button></td>
            <td><button type="button" class="btn btn-outline-secondary fw-bolder" onclick="hapusplgn(${el.idpelanggan})">HAPUS</button></td>
            <td><button type="button" class="btn btn-outline-success fw-bolder" onclick="cartplgn(${el.idpelanggan})">${btncart}</button></td>
            </tr>`;
        });
        out += '</tbody></table>';
        tampil.innerHTML = out;
...
function addplgn() {
    ...
    axios.post("http://localhost:8080/dummyjson-v2/php/insert.php", JSON.stringify(data)).then(function (response) {
        alert(response.data);
        allplgn();
    })
}
...
function tampilplgn(idpelanggan) {
    ...
    axios.post("http://localhost:8080/dummyjson-v2/php/selectupdate.php", JSON.stringify(data)).then(function (response) {
...
function ubahplgn() {
    ...
    axios.post("http://localhost:8080/dummyjson-v2/php/update.php", JSON.stringify(dataPelanggan)).then(function (response) {
        alert(response.data);
        allplgn();
    })
}
...
function hapusplgn(idpelanggan) {
    ...
    axios.post("http://localhost:8080/dummyjson-v2/php/delete.php", JSON.stringify(data)).then(function (response) {
        alert(response.data);
        allplgn();
    })
}
...
function cartplgn(idpelanggan) {
    axios.get("http://localhost:8080/dummyjson-v2/php/order/selectwhere.php/?id=" + idpelanggan).then(function (response) {
...
function checkout(idbarang, harga, barang) {
    ...
    axios.post("http://localhost:8080/dummyjson-v2/php/order/addtocart.php", JSON.stringify(data)).then(function (response) {
